    ...
    public class DraggedLinkElement : BaseLinkElement
    {
        ...
        protected override Tuple<NodeDirection, NodeDirection> GetEndpointDirections()
        {
            ...
            if (SupplierElement == null)
            {
                if (myParent is DraggedLinkElement masterLinkElement)
                {
                    ...
                    return new Tuple<NodeDirection, NodeDirection>(masterLinkElement.GetEndpointDirections().Item1 == NodeDirection.Up ?
                        NodeDirection.Down : NodeDirection.Up, ConsumerElement.DisplayedNode.NodeDirection);
                }
        ...
        private void EndDrag(Point graph_point)
        {
            ...
            if (SupplierElement != null && ConsumerElement != null) //no nulls -> link 2 nodes, or with Ctrl: insert a passthrough between them
            {
                ...
                {
                    graphViewer.AddPassthroughNodeBetween(SupplierElement, ConsumerElement, this.Item);
                }
                else
                {
            ...
            else //at least one null -> add new recipe, or with Ctrl/Ctrl+Shift shortcuts
            {
                ...
                bool droppedOnNonMatchingNode = graphViewer.GetNodeAtPoint(graph_point) != null;
